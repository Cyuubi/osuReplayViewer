...
namespace osuReplayViewer.SoundTouch
{
    class SoundTouch : IDisposable
    {
        ...
        public string VersionString
        {
            get
            {
                if (_versionString == null)
                {
                    ...
                    if (_is64Bit)
                        SoundTouchInterop64.soundtouch_getVersionString2(versionString, versionString.Capacity);
                    else
                        SoundTouchInterop32.soundtouch_getVersionString2(versionString, versionString.Capacity);

                    _versionString = versionString.ToString();
                }
        ...
        {
            if (_is64Bit) return (int)SoundTouchInterop64.soundtouch_receiveSamples(_handle, outBuffer, (uint)maxSamples);
        ...
        public bool IsEmpty
        {
            get
            {
                if (_is64Bit) return SoundTouchInterop64.soundtouch_isEmpty(_handle) != 0;
        ...
        public int NumberOfSamplesAvailable
        {
            get
            {
                if (_is64Bit) return (int)SoundTouchInterop64.soundtouch_numSamples(_handle);
        ...
        public int NumberOfUnprocessedSamples
        {
            get
            {
                if (_is64Bit) return SoundTouchInterop64.soundtouch_numUnprocessedSamples(_handle);
        ...
        {
            if (_is64Bit) return SoundTouchInterop64.soundtouch_getSetting(_handle, SoundTouchSettings.UseAaFilter);
        ...
        {
            if (_is64Bit) return SoundTouchInterop64.soundtouch_getSetting(_handle, SoundTouchSettings.UseQuickSeek);
